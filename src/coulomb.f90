...
!HL Defining w_ryd here so that we can test the pade evaluation on a denser grid.
...
!HL Test everything
           !write(1000+mpime,'(2f12.8)') scrcoul(ig,igp,iw,nspin_mag)
         enddo
...
!    enddo
!    do iw = 1, nwcoul
!       write(6,'(4x,"PADE INVEPS: ",5f11.2)') w_ryd(iw)*RYTOEV, scrcoul (1,1,iw,nspin_mag)
!    enddo
!    endif
    enddo !enddo on igp
   endif !on qg2 > 0 
