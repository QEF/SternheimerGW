...
!
  ...
  COMPLEX(DP) :: gr_A(npwx), rhs(npwx)
  COMPLEX(DP) :: gr_N(npwx), gr(npwx), ci, cw, green(ngmsig,ngmsig)
...
!  G^{NA}v(r,r') component of the self-energy.
...
!@10TION
! Here it becomes important that all the valence band eigenvalues are < 0 for the frequency formalism etc.
  
    et(:,2) = et(:,2) - (6.58D0/13.605)
...
! HL 05/07/11 except in the expressions for the analytic component of the green's function the eigenvalue
! doesn't show up anywhere.

       etc(:, :) = CMPLX( 0.0d0, 0.0d0, kind=DP)
       cw = CMPLX( w_ryd(iw), 0.2d0, kind=DP)

!      cw = CMPLX( 0.0d0, w_ryd(iw), kind=DP)
!      cw(1) = CMPLX( w_ryd(iw), eta, kind=DP)
...
! green_linsys in sgw passes cdummy as the values for eqv: i.e. 
! the occupied manifold is set to zero...
! i.e. all the projectors onto the empty state should be zero.
! also ikq is set to zero.
...
!      Evaluate the G_{N} part of the Green's function.
...
!      SGW  gr_N(ig) = gr_N(ig) + tpi * ci * conjg( evq(ig,ibnd) ) * evq(igp,ibnd) * dirac
      ...
      enddo
      gr = gr_A + gr_N
!     write(6,'(3f15.10)') gr_A
      do igp = 1, ngmsig
         green (ig,igp) = gr (igp)
!        write(6, '(3f15.10)') green (ig, igp)
