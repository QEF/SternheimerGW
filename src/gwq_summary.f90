...
!
!-----------------------------------------------------------------------
subroutine gwq_summary
  !-----------------------------------------------------------------------
  !
  !    This routine writes on output the quantities which have been read
  !    from the punch file, and the quantities computed in the gwq_setup
  !    file.
  !
  ...
  USE control_gw,    ONLY : lgamma_gamma, lnoloc, lrpa, zue, epsil, ldisp, &
                            nmix_gw, alpha_mix, tr2_gw, zeu
  USE freq_gw,       ONLY : fpol, nfs, fiu
  ...
  WRITE( stdout, 100) title, ibrav, alat, omega, nat, ntyp, &
       ecutwfc, ecutwfc * dual, tr2_gw, alpha_mix (1), &
       nmix_gw
100 format (/,5x,a75,/,/,5x, &
 ...
 !HL WRITE( stdout, '(/,5x,"Computing dynamical matrix for ")')
 !HL 
  WRITE( stdout, '(/,5x,"Computing q-point: ")')
