...
!r: in case we want to check the convergence of the residuals of the shifted system
!             beta = ZDOTC (ndim, r(1), 1, r(1), 1)
              do iw = 1, nfreq 
                 u_sig(:,iw) = r(:)
                 r_sig(:,iw) = r(:)
              enddo
...
!with rhs as delta <rt,r> is always one on the first iteration.
         do iw = 1, nfreq
!HLA
        anorm(iw) = sqrt ( abs ( ZDOTC (ndim, r_sig(1,iw), 1, r_sig(1,iw), 1)  ) )
!       if (anorm_pos.lt.ethr) conv(iw) = .true.
...
!The shifted residual tells us if we are converged. 
         do iw = 1, nfreq
