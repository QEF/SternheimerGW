...
MODULE gw_restart
  ...
  PRIVATE
  !
  PUBLIC :: gw_writefile, gw_readfile, init_status_run, check_status_run, &
            destroy_status_run
  !
  ...
  CONTAINS
    !
    !------------------------------------------------------------------------
    SUBROUTINE gw_writefile( what, irr )
      !------------------------------------------------------------------------
      !
      USE global_version,       ONLY : version_number
      USE control_gw,           ONLY : current_iq, done_bands, ldisp, trans
      ...
      IF ( ionode ) THEN
         ...
         ELSE
            CALL errore('gw_writefile','unknown what',1)
         ENDIF
         !
      END IF
      !
      ...
      IF ( ionode ) THEN  
         !
         ! ... here we start writing the gw-punch-file
         !
!-------------------------------------------------------------------------------
! ... HEADER
!-------------------------------------------------------------------------------
...
!
            CALL write_status_gw(current_iq, done_bands)
!-------------------------------------------------------------------------------
! ... CONTROL 
!-------------------------------------------------------------------------------
         !
            CALL write_control_gw( ldisp, trans)
         !
!-------------------------------------------------------------------------------
! ... Q POINTS
!------------------------------------------------------------------------------
...
! point in which the gwonon code has written the recover file: where_rec 
...
!
      ...
      CONTAINS
         ...
         SUBROUTINE write_partial_gw()
            USE modes, ONLY : nirr, npert, u, name_rap_mode, nsymq
            USE partial, ONLY : done_irr
            USE control_gw, ONLY : current_iq, epsil, trans, elgw, zue, lgamma, where_rec, rec_code
...
!   Save the current flags
            RETURN
         END SUBROUTINE write_partial_gw
    END SUBROUTINE gw_writefile 

    SUBROUTINE write_control_gw( ldisp, trans)
      !------------------------------------------------------------------------
      !
      IMPLICIT NONE
      ...
      !
      RETURN
    END SUBROUTINE write_control_gw

    SUBROUTINE write_status_gw(current_iq, done_bands)
      !------------------------------------------------------------------------
      !
      IMPLICIT NONE
      ...
      !
      RETURN
    END SUBROUTINE write_status_gw
    !
