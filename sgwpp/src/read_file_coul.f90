...
SUBROUTINE read_file_coul()
  ...
  USE pw_restart_coul,      ONLY : pw_readfile
  ...
  io_level = 1
  print*, trim(tmp_dir)
  !CALL open_buffer ( iunwfc, 'wfc', nwordwfc, io_level, exst, exst_mem, tmp_dir )
  ...
  !
END SUBROUTINE read_file_coul
!
!----------------------------------------------------------------------------
SUBROUTINE read_xml_file_coul()
  !----------------------------------------------------------------------------
  !
  ...
  !
  CALL set_dimensions()
 !CALL realspace_grid_init ( dfftp, dffts, at, bg, gcutm, gcutms )
  ...
  !!ALL poolscatter( nbnd, nkstot, wg, nks, wg )
  ! ... check on symmetry
  !
  ...
  CONTAINS
    ...
    !
END SUBROUTINE read_xml_file_coul
