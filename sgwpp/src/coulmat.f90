...
SUBROUTINE coulmats(vcnknpkp)
  ...
  USE control_coulmat,      ONLY : nbndmin, ngcoul, do_lind
...
!nks should be local pool index for kpoints.!
!need to make sure the loop is only over the local k-points...
   do ik = 1, nks
...
!LOOP over bands calculate f functions etc.
              ...
              do ig = 1, ngcoul 
                 do igp = 1, ngcoul
                   vcnknpkp(ik, ikp, ibnd, jbnd) = vcnknpkp(ik, ikp, ibnd, jbnd) + &
                                                   conjg(fnknpkp(nls(ig)))*vc(ig,igp)*fnknpkp(nls(ig))
                 enddo
              enddo
            else
              do ig = 1, ngcoul 
