...
a comprehensive planewave implementation would be the inclusion of subroutines 
to deal with nonlocal pseudopotentials of norm-conserving\cite{ncpp} or ultrasoft\cite{uspp} 
form. The effective exchange and correlation potential $V^{\rm xc}[n]$ has been determined within
the local-density approximation to DFT following Ref.\ \onlinecite{LDA}.
The EPM single-particle Hamiltonian can therefore be rewritten as
  \begin{equation}
  \H_{\rm EPM} = -\frac{\nabla^2}{2} + V^{\rm ps}_{\rm EPM} + V^{\rm Ha}[n] + V^{\rm xc}[n],
  \end{equation}
with an effective unscreened empirical pseudopotential $V^{\rm ps}_{\rm EPM}$ given by:
  \begin{equation}
  V^{\rm ps}_{\rm EPM} = V_{\rm EPM} - V^{\rm Ha}[n] - V^{\rm xc}[n],
  \end{equation}
$V_{\rm EPM}$ being the pseudopotential originally proposed in Ref.\ \onlinecite{cohen_berg}.
