...
We define the non-interacting Green's function following Ref.\ \onlinecite{hl}:
  ...
  G(\r,\rp;\w) = 2\sum_n \frac{\psi_n(\r)\psi_n^\star(\rp)}{\w-\E_n-i\delta_n},
  \end{equation}
the sum extending over all states and the factors of 2 accounting for the
spin degeneracy. The infinitesimal $\delta_n$ is positive ($\delta_n=\delta$) 
for occupied states and negative ($\delta_n=-\delta$) for unoccupied states.\cite{hl,hl86}
...
sum in Eq.~(\ref{eq.green.1}) into occupied and unoccupied states:
  ...
  G(\r,\rp;\w) = 2\sum_v \frac{\psi_v(\r)\psi_v^\star(\rp)}{\w-\E_v-i\delta}
  + 2\sum_c \frac{\psi_c(\r)\psi_c^\star(\rp)}{\w-\E_c+i\delta}.
  \end{equation}
As in the case of the screened Coulomb interaction,
it is convenient to regard the Green's function as a parametric
function of the the first space variable
and of the frequency: $G_{[\r,\w]}(\rp) = G(\r,\rp;\w)$.
If we act of both sides of Eq.\ (\ref{eq.green.2}) with the operator
$\H-\w-i\delta$ we find immediately:
  \begin{equation}\label{eq.green.3}
  (\H-\w-i\delta) G_{[\r,\w]} = -2 b_{[\r,\w]},
  \end{equation}
with
  \begin{equation}\label{eq.green.3b}
  b_{[\r,\w]}(\rp) = \delta(\r,\rp) + i\,2\delta \sum_v \frac{\psi^\star_v(\r)}{\w-\E_v-i\delta} \psi_v(\rp).
  \end{equation}
In order to arrive at
Eqs.\ (\ref{eq.green.3}),(\ref{eq.green.3b}) we made use of time-reversal symmetry and the
completeness relation $\sum_n \psi_v(\r)\psi_v^\star(\rp) = \delta(\r,\rp)$.
In practice, by using Eqs.\ (\ref{eq.green.3}),(\ref{eq.green.3b}) we can determine the propagation
from a point $\r$ to every other point $\rp$ by solving a inhomogeneous
linear system. As Eq.\ (\ref{eq.green.3b}) does not contain unoccupied
...
normalization:
  ...
  G(\r,\rp;\w) = \frac{2}{N_\k}\sum_{n\k} \frac{\psi_{n\k}(\r)\psi_{n\k}^\star(\rp)}{\w-\E_{n\k}-i\delta_{n\k}},
  \end{equation}
...
of the Bloch waves ${\rm exp}[-i(\k+\G)\cdot\r]$ and ${\rm exp}[i\k\cdot\rp]$:
  ...
  G_{[\r,\w]} (\rp) = \frac{1}{\Omega N_\k}  \sum_{\k\G} g_{[\k,\G,\w]}(\rp)
   {\rm e}^{-i(\k+\G)\cdot\r} {\rm e}^{i\k\cdot\rp}.
  \end{equation}
Following the same convention we expand the wavefunctions as
$\psi_{n\k}(\rp)=\Omega^{-1/2}\sum_\Gp {\rm e}^{i(\k+\Gp)\cdot\rp} u_{n\k}(\Gp)$.
Using these definitions and exploiting again time-reversal symmetry
Eq.\ (\ref{eq.green.3}) becomes
  \begin{equation}\label{eq.green.3.g}
   (\H_\k-\w-i\delta)  g_{[\k,\G,\w]}  =  -2\, b_{[\k,\G,\w]},
  \end{equation}
with 
  \begin{equation}
 b_{[\k,\G,\w]}(\rp)  =  {\rm e}^{i\G\cdot\rp} +  
 i\,2\delta \sum_v \frac{\sqrt{\Omega}\,u^\star_{v\k}(\G)}{\w-\E_{v\k}-i\delta} u_{v\k} (\rp).
  \end{equation}
In deriving the last equation we made use of time-reversal symmetry, yielding
...
we obtain an entire row $\Gp$ of the Green's function $g_{[\k,\G,\w]}(\Gp)$.

\begin{itemize}
\item 
The following equation is only for myself - to show consistency with the code:
\begin{equation}
 b_{[\k,\G,\w]}(\Gp)  =  \delta_{\G\Gp} + 
 i\,2\delta \sum_v \frac{u^\star_{v\k}(\G)u_{v\k} (\Gp)}{\w-\E_{v\k}-i\delta}. 
  \end{equation}
Now G and W should be perfectly consistent in the code and here in the paper - and 
with the same convention.
\item
problem with ill-conditioning
\item
good scaling.
\end{itemize}
