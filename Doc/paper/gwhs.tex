...
%of the starting hamiltonian by GW. 
...
\begin  {figure}
\begin  {center}
\includegraphics[width=6.5cm]{fig.diel.eps}
\end    {center}
...
\begin  {figure}
\begin  {center}
\includegraphics[width=7.5cm]{fig.pade.eps}
\end    {center}
...
(1) We now move on to the frequency-dependence of the dielectric matrix:
comparison of frequency dependence wrt plots in steve's paper.
(2) At this point we discuss the quality of the Pad\'e approximant.

(3) Typical number of iterations for CF convergence, SCF convergence (mention which mixing),
w and w/o preconditioning, as a function of accuracy, and condition number estimated with simple equation.
green's function: typical convergence parameters.

Need to specify size of dielectric matrix - note we use 4 times the cutoff
specified in input.
We used a cutoff of 5 Ry for the screened Coulomb interaction (actually 4.4) - this requires dielectric matrices 
of size 51x51. The resulting self-energy is 17.6 Ry, this is exagerated but still we want to demonstrate
large diel matrices. 
We need also to specify the number of bins in the frequency integration, the smearing, and the ranges for G, W, Sigma.


\begin  {figure}
\begin  {center}
\includegraphics[width=6cm]{fig.green}
\end    {center}
\caption{\label{fig.green}
...
\begin  {figure}
\begin  {center}
\includegraphics[width=6cm]{fig.sigma.r}
\end    {center}
\caption{\label{fig.sigma.r}
        a fancy 2D self-energy plot to show that we do S(G,G') 
        (maybe at w=0 and w=Eg)
        }
\end    {figure}

Upward shift of VBT by 0.44 eV.
