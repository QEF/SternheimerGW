...
is explicitly taken into account by performing calculations along the imaginary
frequency axis, followed by the analytic continuation to real frequencies
...
using silicon as a test case. We crytically examine the advantages and
...
has already been discussed in Refs.\ \onlinecite{fleszar-resta} and \onlinecite{kunc-tosatti},
respectively, 
...
advantage of the {\it multishift} linear system solver of Ref.\ \onlinecite{frommer}
...
Also in this case we exploit the multishift method of Ref.\ \onlinecite{frommer}
...
was motivated by the need of making contact with existing literature on dielectric
...
Coulomb interaction in reciprocal space becomes:
  ...
  \end{equation}
In the long-wavelength limit $\q\!\rightarrow\!0$ the head ($\G=\Gp=0$) of the truncated
...
of Ref.~\onlinecite{jacobs} (cBiCG), as described in Appendix \ref{app.cbcg}.
...
Since the method of Ref.\ \onlinecite{hl86} calculates the dielectric matrix and then performs a matrix inversion,
...
then our proposed DFPT approach would be more convenient that the standard sum-over-states
approach.

The above analysis shows that the main bottlenecks of our DFPT-$GW$ method 
...
our calculations and Ref.~\onlinecite{balde_tosa} supports the validity of our approach.
...
Sec.\ \ref{sec.diel.nscf} in order to compare our results with Ref.\ \onlinecite{cohen.diel1}.
% 12-12-12 000 5Ry.
...
at long wavelength with the data reported in Ref.\ \onlinecite{hl86}. 
...
linear system in Eq.\ (\ref{eq.linsys.1}) becomes progressively better conditioned
...
Charge-sloshing effects are atenuated by using the potential mixing method proposed in
 Ref.\ \onlinecite{johnson}, appropriately modified to deal with complex potentials. 
...
with the calculations of of Ref.\ \onlinecite{blochl} performed within DFT/LDA 
...
calculations including PAW core-reconstruction of Ref.\ \onlinecite{blochl}.
...
close to those obtained in Ref.\ \onlinecite{hl86} using {\it ab-initio} pseudopotentials
...
to the corresponding values of -12.54 eV and -5.28 eV of Ref.~\onlinecite{hl86}.
...
point and for the conduction band edge of silicon with the results of Refs.\ \onlinecite{hl86,blochl}.
...
such agreement is rather surprising.
...
wavefunctions are negligible. Such diagonal approximation is not always justified, nonetheless
we decided to adopt Eq.\ (\ref{eq.specfun}) in order to be consistent with Ref.\ \onlinecite{blochl}.
...
DFPT with the results obtained in Ref.\ \onlinecite{balde_tosa}
...
or ultrasoft\cite{uspp} pseudopotentials should not present and difficulties
...
In summary, we proposed a new methodology for performing $GW$ calculations
...
As the cBiCG algorithm introduced in Ref.\ \onlinecite{jacobs} 
...
as suggested in Ref.\ \onlinecite{spacetime}. 
...
%Our present analysis supports the view 
