...
spin degeneracy. The infinitesimal $\eta_n$ is positive ($\eta_n=\eta$) 
...
$\H-\w^+$, having set $\w^+ = \w+i\eta$, we find immediately:
  \begin{equation}\label{eq.green.3}
  (\H-\w^+) G_{[\r,\w]}(\rp) = -2 [ \delta(\r,\rp) + p_{[\r,\w]}(\rp) ],
  \end{equation}
with
  \begin{equation}\label{eq.green.3b}
  p_{[\r,\w]}(\rp) = 2\pi i \sum_v (\w^+-\E_v)\delta(\w-\E_v) \psi^\star_v(\r)\psi_v(\rp).
  \end{equation}
...
We also used the Lorentzian representation of the Dirac's delta function
$\pi\delta(x)=\eta/(x^2+\eta^2)$ for small $\eta$.
Whenever $\w$ is in the conduction bands energy range ($\w\ge\E_c^{\rm min}$) 
Eq.\ (\ref{eq.green.3b}) vanishes and can be ignored.
On the other hand, when $\w$ is in the valence bands energy range ($\w\le\E_v^{\rm max}$),
$p_{[\r,\w]}(\rp)$ exhibits poles associated with the occupied states.
...
In the latter case ($\w\le\E_v^{\rm max}$) we cross branch cuts when we move
from $\w^+=\w+i\eta$ to $\w$ and a proper account of the pole structure 
through Eq.\ (\ref{eq.green.3b}) is necessary. A detailed discussion of this
...
and the driving term $p_{[\k,\G,\w]}(\rp)  = \sum_\Gp {\rm e}^{i(\k+\Gp)\cdot\rp} p_{[\k,\G,\w]}(\Gp)$,
Eqs.~(\ref{eq.green.3}),(\ref{eq.green.3b}) become
  \begin{equation}\label{eq.green.3.g}
   (\H_\k-\w^+)  g_{[\k,\G,\w]}(\Gp)  =  -2 [ \delta_{\G\Gp} + p_{[\k,\G,\w]}(\Gp)],
  \end{equation}
  \begin{equation} \label{eq.green.4b.g}
  p_{[\k,\G,\w]}(\Gp)  =  
  2\pi i \sum_v (\w^+-\E_{v\k})\delta(\w-\E_{v\k}) u^\star_{v\k}(\G)u_{v\k} (\Gp).
  \end{equation}
...
when $\w\ge\E_c^{\rm min}$, within this frequency range it is possible
to perform the simultaneous calculation of all the frequency responses using multishift
methods (cf.\ Appendix \ref{app.multishift}). This procedure is extremely
