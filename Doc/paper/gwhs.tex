...
and the factor of 2 taking into account the spin degeneracy.\cite{note.w.even}
...
This is exactly the variation of the density-matrix $\Delta\rho(\r,\rp;\w)$
...
By combining the previous equations we can rewrite Eqs.\ (\ref{eq.w}) and (\ref{eq.p}) as
  ...
  \end{equation}
At this stage, the calculation of the screened interaction is performed
by solving the linear systems self-consistently (the changes in the wfs depend
implicitely on the screened perturbation). The machinery required for this
calculations has been developed within the frameworkd of DFPT for lattice-dynamics.
Interestingly, in the case of purely imaginary frequencies $\w=i\beta$,
we need to solve only one linear systemi because $-\w^\star=\w$, 
as in the static case (for purely real frequencies we need to solve two systems).
...
DISCUSS THE PHYSICS OF SCREENING A POINT CHARGE.
