...
$\H-\w-i\delta$ we find immediately:
  ...
  (\H-\w-i\delta) G_{[\r,\w]} = -2\Big[ \delta_{[\r]} + i \sum_v \frac{2\psi_v(\r)\delta}{\w-\E_v-i\delta} \psi_v^\star\Big],
  \end{equation}
where we used the definition $\delta_{[\r]}(\rp)=\delta(\r,\rp)$. In order to arrive at
Eq.\ (\ref{eq.green.3}) we made use of time-reversal symmetry and the
completeness relation $\sum_n \psi_v(\r)\psi_v^\star(\rp) = \delta(\r,\rp)$.
In practice, by using Eq.\ (\ref{eq.green.3}) we can determine the propagation
...
Need to explain ill-conditioning of the linear system, but that the situation
is still ok if we use TPA.
