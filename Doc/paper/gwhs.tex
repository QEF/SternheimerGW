...
Now we introduce the following convention for the Fourier transform of our functions:
  \begin{eqnarray}
  f(\r,\rp;\w) & = & \frac{1}{N_\q} \sum_{\q\G} f_{[\q\G\w]}(\rp) {\rm e}^{i\q(\rp-\r)} {\rm e}^{-i\G\r}, \\
...
In particular:
  \begin{equation}
  \Delta V_{[\r,\w]} = \frac{1}{N_\q} \sum_{\q\G} \Delta V_{[\q\G\w]}(\rp) {\rm e}^{i\q(\rp-\r)} {\rm e}^{-i\G\r}.
  \end{equation}
...
we find two wavevectors $\q$ and $\qp$:
  \begin{equation}
  \Delta \psi^\pm_{v\k[\r,\w]} (\rp) = \frac{1}{N_\q^2} \sum_{\q\qp\G} \Delta u^{\qp\pm}_{v\k[\q\G\w]} (\rp) 
...
therefore
  \begin{equation}
  \Delta \psi^\pm_{v\k[\r,\w]} (\rp) = \frac{1}{N_\q} \sum_{\q\G} \Delta u^{\k+\q,\pm}_{v\k[\q\G\w]} (\rp)
  {\rm e}^{i(\k+\q)\cdot\rp} {\rm e}^{-i(\q+\G)\cdot\r},
  \end{equation}
The charge density can now be rewritten as:
  \begin{equation}
  \Delta n_{[\r,\w]} = \frac{1}{N_\q} \sum_{\q\G} \Delta n_{[\q\G\w]}(\rp) {\rm e}^{i\q(\rp-\r)} {\rm e}^{-i\G\r},
  \end{equation}
hence
  \begin{equation}
...
The induced Hartree potential can then be written as (NOT SURE IF I AM MISSING A PREFACTOR)
  \begin{equation}
...
therefore the screened Coulomb interaction reads
  ...
  \end{equation}
and the inverse dielectric matrix is
  \begin{equation}
  \epsilon^{-1}_{\G\Gp}(\q;\w) = \delta_{\G\Gp} + \Delta n_{[\q\G\w]}(\Gp) .
  \end{equation}
The convention used for the Coulomb potential is
  \begin{equation}
  v(\r,\rp) = \frac{1}{N_\q} \sum_{\q\G} v(\q+\G) {\rm e}^{i\q\cdot(\rp-\r)} {\rm e}^{i\G\cdot(\rp-\r)}.
  \end{equation}
