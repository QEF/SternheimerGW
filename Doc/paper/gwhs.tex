...
diagrams\cite{york,bruneval} and the description of the frequency-dependent
...
found in Refs.\ \onlinecite{hl,gunnarsson,reining,rinke}.
...
and to perform $GW$ calculations by employing occupied electronic states only,\cite{reining-sternheimer,umari1,gygi}
or only a limited number of unoccupied states.\cite{bruneval-gonze}

In this work we propose a new approach to $GW$ calculations entirely based on
density functional perturbation theory (DFPT).\cite{baroni.rmp}
In practice we adopt the principles of DFPT in order to (i) determine the {\it frequency-dependent 
screened Coulomb interaction} directly
...
The use of the Sternheimer equations for calculating the polarizability
in the random-phase approximation (RPA) or even the inverse dielectric matrix 
have already been discussed in Refs.\ \onlinecite{fleszar-resta,kunc-tosatti} 
within the context of supercell calculations (without DFPT). After the advent
of DFPT, the authors of Ref.\ \onlinecite{reining-sternheimer} proposed the
application of non self-consistent Sternheimer methods to the calculation
of the dielectric matrix.
The main aim of the present work is to demonstrate the feasibility of $GW$ calculations
based on {\it self-consistent} DFPT. In addition we investigate the possibility
of fast evaluations of the frequency dependence of the screened Coulomb interaction
...
occupied and unoccupied states, and the factor 2 accounts for the
...
In addition, by using time-reversal symmetry we obtain $P(\r,\rp;-\w)=P^\star(\r,\rp;\w)$,
therefore $W(\r,\rp;-\w)=W^\star(\r,\rp;\w)$.
...
We define the non-interacting Green's function following Ref.\ \onlinecite{hl}:
  ...
  \end{equation}
the sum extending over all states. The spin degeneracy factor is not included
since only states with the same spin couple when forming the matrix elements
of teh self-energy.\cite{hl86}
...
A detailed discussion of this aspect is provided in Ref.\ \onlinecite{hl}.
