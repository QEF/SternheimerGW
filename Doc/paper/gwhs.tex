...
INTRODUCE THE WHOLE NOTATION HERE.

\subsubsection{Screened Coulomb interaction}\label{sec.coulomb.scaling}

In this section we refer to the planewaves implementation outlined in Sec.\ \ref{sec.coulomb.g},
and for the sake of clarity we consider a $\Gamma$ point sampling of the Brillouin zone
(without loosing generality), and a plasmon-pole approximation for the frequency-dependence.
Equation (\ref{eq.linsys.4}) needs to be solved for every planewave perturbation
$\G$ ($\q=0$ and $\w=0$ in this simplified discussion). A smooth cutoff $E_{\rm cut}^{\rm s}$ 
(typically smaller than the wavefunction cutoff) corresponds to $N_\G^{\rm s}$ perturbations.
Each solution of Eq.\ (\ref{eq.linsys.4}) requires two cBiCG minimizations
(one for $-\w$ and the other for $+\w$), and each cBiCG minimization consists of two CG sequences.
The most expensive step in each CG step is the application of the Hamiltonian operator
to the previous search direction. In a planewaves basis the most time-consuming step in the
latter operation is the Fourier transform of the wavefunctions to real-space (in order
to calculate the product with the local potential) and back. Using fast-Fourier-transform
algorithms this can be achieved in $4 N_\r^{\rm den} {\rm log} N_\r^{\rm den}$ operations,\cite{frigo}
$N_\r^{\rm den}$ being the number of real-space grid points necessary to represent the 
electron density (given by 4 times the wavefunction cutoff).
If in average the CG minimization requires $N_{\rm CG}$ steps,
the self-consistent procedures requires $N_{\rm SCF}$ iterations, and we need to perform
the calculation for $N_v$ occupied electronic states, then the total cost
of the entire calculation is approximately given by 
   \begin{equation}\label{eq.scaling.1}
   N^{\rm DFPT}_{\rm flops} = 8 N_{\rm CG} N_{\rm SCF} N_\G^{\rm s} [ N_v ( 4 N_\r^{\rm den} {\rm log} N_\r^{\rm den}) ]
   \end{equation}
floating point operations.
As $N_\G^{\rm s}$, $N_v$, and $N_\r$ are all proportional 
to the size of the systems as measured by the number of atoms $N_{\rm at}$,
the overall scaling of this procedure is $N_{\rm at}^3 {\rm log} N_{\rm at}$.

For comparison it is useful to estimate the scaling of tstandard $GW$ calculations
based on the expansion over unoccupied states.\cite{hl86} 
The calculation of the irreducible polarization requires the optical matrix elements
of each of the $N_v$ occupied states with each of the $N_c$ unoccupied states.
These matrix elements are actually computed by using Fourier transforms of $\psi_c^\star(\r)\psi_v(\r)$,
therefore this requires $N_v + N_c$ FFTs to real-space, mutliplication, and $N_v N_c$ FFTs back
to reciprocal space. Each FFT must be performed on the $N_\r^{\rm den}$ grid, therefore
the total cost is 
   \begin{equation}
   N^{\rm HL86}_{\rm flops} = N_c  [N_v (4 N_\r^{\rm den} {\rm log} N_\r^{\rm den}) ]
   \end{equation}
 floating point operations.
...
For the purpose of a fair comparison we set now $N_{\rm SCF}=1$ in Eq.\ (\ref{eq.scaling.1}),
meaning that we calculate the dielectric matrix as described in Sec.\ \ref{sec.diel.nscf} (instead of the
inverse dielectric matrix).
This also brings in the advantage that, by using the multishift method, 
only one calculation is required for $\w=0$ instead of two and that in this case the two CG sequences do coincide.
As a result a factor 4 drops out of the prefactor and 
the ratio between the costs of the DFPT approach in a planewaves
implementation and the standard method is (assuming a perfectly well-conditioned system with $\kappa=1$):
%   \begin{equation}
%   \frac{N^{\rm DFPT}_{\rm flops}}{N^{\rm HL86}_{\rm flops}} = \frac{2 N_{\rm CG} N_\G^{\rm s}}{N_c}.
%   \end{equation}
   \begin{equation}\label{eq.ratio.flops}
   \frac{N^{\rm DFPT}_{\rm flops}}{N^{\rm HL86}_{\rm flops}} = \frac{N_\G^{\rm s} {\rm log} (2/\varepsilon)}{N_c}.
   \end{equation}
As an example, for a relative accuracy of $\varepsilon=10^{-5}$ 
we find this ratio to be $\simeq 12 N_\G^{\rm s}/N_c$.
In the case of silicon, with $E_{\rm cut}^{\rm s}=10$ Ry we have $N_\G^{\rm s}=137$, therefore
the DFPT approach becomes convenient when more than 1650 unoccupied states are used.
This is rarely the case as most calculations reported to date use only a few hundredths of 
unoccupied electronic states.
Of course the accuracy of the standard sum-over-states expression is more difficult to
assess, and a convergence on 5 significant digits is probably not achieved with only a few
hundreths of electronic states. It remains however that an implementation based on planewaves
is at least as expensive as the standard approach.
...
the whole self-energy $\Sigma_{\G\Gp}(\k;\w)$, while the standard approach would 
only provide the matrix elements of the self-energy on a subset of states which is
typically of the order of $N_v$. Therefore if we performed a comparison based on the
same amount of output information, we should use $N_c N_\G^{\rm s}$ in Eq.\ (\ref{eq.ratio.flops})
instead of $N_c N_v$. In this case the ration above would change into
  \begin{equation}
  \frac{N^{\rm DFPT}_{\rm flops}}{N^{\rm HL86}_{\rm flops}} = \frac{N_v {\rm log} (2/\varepsilon)}{N_c},
  \end{equation}
and for $\varepsilon=10^{-5}$ in silicon we would have $\simeq 64/N_c$.
This clearly shows that, {\it if the entire self-energy is needed} (as opposed to a few matrix elements),
{\it then our proposed DFPT approach is more convenient that the standard sum-over-states
procedure}.

What we learn from the above analysis is that the main bottlenecks of the calculation are
the FFT for the application of the Hamiltonian and the large basis sets adopted.
Therefore one possibility to improve over the present method is to keep the general
theory developed in Sec.\ \ref{sec.coulomb} and move to either a (i) real-space 
method where the application of the Hamiltonian scales linearly with system size,
or to (ii) a local basis set.
In both cases it seems that a calculation scaling with $N_{\rm at}^3$ should be feasible,
and with a more favourable prefactor in the case of a local basis set.
...
the optimal  number of iterations (for a relative accuracy $\epsilon=10^{-10}$) 
...
dependence, in line with the observation of Ref.\ \onlinecite{blochl}
...
already proposed in Ref.\ \onlinecite{reining-sternheimer}.
