...
If we now apply the operator $\H-\w + i\eta$ to both sides of Eq.\ (\ref{eq.green.split.2})
and we use the completeness relation $\sum_n \psi_n(\r)\psi_n^\star(\rp) = \delta(\r,\rp)$
we find immediately:
...
($4/3\pi R_{\rm c}^3 = N_\q \Omega$), then we would have obtained
  \begin{equation}
  v_{\rm tr} (\q+\G) = \frac{4\pi e^2}{|\q+\G|^2} (1-\cos |\q+\G| R_{\rm c}),
  \end{equation}
...
we discover that for each component $\Delta v_{[\q,\G,\w]}$ of the
perturbing potential having a Bloch wavevector $\q$,
the associated variations of the wavefunctions must carry 
a Bloch wavevector $\k+\q$:
  \begin{equation}
...
As a result the linear system Eq.\ (\ref{eq.linsys.1}) simplifies to
  \begin{equation}\label{eq.linsys.4}
  (\H_{\k+\q}-\E_{v\k}\pm\w) \Delta u^\pm_{v\k[\q,\G,\w]}  = -(1-\P^{\k+\q}) \Delta v_{[\q,\G,\w]} u_{v\k},
  \end{equation}

NEED TO SAY WHAT IS Hk AND Pk+q

The induced charge density corresponding to the Bloch wave ${\rm exp}(-i\q\cdot\r)$ 
reads
  \begin{equation} \label{eq.deltan.bloch}
  \Delta n_{[\q,\G,\w]} = \frac{2}{N_\k}\sum_{v\k\s} u_{v\k}^\star  \Delta u^\s_{v\k[\q,\G,\w]}.
  \end{equation}
This result is similar to the case of standard DFPT.\cite{baroni.rmp} 
...
%
If the cell-periodic function $\Delta n_{[\q,\G,\w]}$ is expanded in plane waves, 
%$\Delta n_{[\q,\G,\w]}(\rp)=\sum_\Gp {\rm e}^{i\Gp\cdot\rp} \Delta n_{[\q,\G,\w]}(\Gp)$
the screened Coulomb interaction Eq.\ (\ref{eq.w.dfpt}) acquires the following simple
form: 
  \begin{equation}
...
The calculation must be repeated for every perturbation, i.e.\ for every 
$\q$, $\G$, and $\w$.
%
...
perturbation ${\rm e}^{i(\q+\G)\cdot\r}$. This idea has been discussed in Ref.\ \onlinecite{kunc-tosatti}
...
The latter possibility has already been discussed in Ref.\ \onlinecite{reining-sternheimer}.
...
to the real axis using Pad\'e approximants.\cite{pade1,pade2,blochl}
