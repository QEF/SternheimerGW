...
$\Sigma^{\rm ex}=Gv$ and a Coulomb term $\Sigma^{\rm c}=G(W-v)$.\cite{pade3}
...
In particular, it can be shown that the $\Sigma^{\rm c}(\w)$ must decay to zero
as $\w^{-1}$ at large frequency.

MAYBE WE CAN TRY TO SHOW THAT THE ERROR SCALES AS wc/wp.

QUI BISOGNA RISCRIVERE UN PO MEGLIO

In order to integrate the exchange term we observe that $\Sigma^{\rm ex}=G^{\rm A}v + G^{\rm N}v$
and that the integration of the term $G^{\rm A}v$ yields a vanishing contribution as
the poles of $G^{\rm A}$ lie in the lower half of the complex plane.
On the other hand, the integration of $G^{\rm N}v$ gives a frequency-independent contribution.
Had we truncated the integral along the real axis, the $\Sigma^{\rm ex}$ term
would have acquired a spurious frequency dependence associated with
the semicircle closing the complex integration path.
In summary, the correct way to perform the integration is to evaluate numerically
