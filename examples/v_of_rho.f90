...
SUBROUTINE v_of_rho( rho, rho_core, rhog_core, &
  ...
  !
  USE kinds,            ONLY : DP
  USE gvect,            ONLY : nrxx, ngm, nr1, nr2, nr3
...
SUBROUTINE v_xc( rho, rho_core, rhog_core, etxc, vtxc, v )
  ...
  ! ... local variables
  !
  REAL(DP) :: rhox, arhox, zeta, amag, vs, ex, ec, vx(2), vc(2), rhoneg(2), rsum
    ! the total charge in each point
    ! the absolute value of the charge
    ! the absolute value of the charge
    ! local exchange energy
...
rsum = 0.d0
do ir = 1, 1728
read(300,*) rho%of_r(ir,1)
rsum = rsum + rho%of_r(ir,1) * omega/1728.d0
enddo
write(6,*) 'read density from file', rsum
!@@@@@
