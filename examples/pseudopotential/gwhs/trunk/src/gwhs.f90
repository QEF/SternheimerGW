  ...
  real(dbl) :: rvec(3), ui, uj, uk, wgreenmin, wgreenmax, w0mw, w0pw, x,y, z, gg, v1,v2
  real(dbl) :: gcutm, gcutms, arg, wp, tmp
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  !
  ! v(G) from SCF calculation
  ! it is important to load g and nl since the ordering
  ! (and hence the fft correspondence) is slightly different
  !
  vr = czero
...
!  do ig = 1, ngm
!    read(100,*) igp, (g(ipol,ig),ipol=1,3), gg, nl(ig), v1, v2
!    vr(nl(ig)) = dcmplx (v1,v2)
!  enddo
...
!@  call cfft3 ( vr, nr1, nr2, nr3,  1)
  ...
  ! local potential
!@  vr = vr - eshift
!  vr = vr - 6.1198040345371920/13.6058
  !
...
rewind(44)
do iq = 1, 165
  read (44,*) xq(1,iq), xq(2,iq), xq(3,iq) 
enddo
...
!@    call eigenstates2 ( xq(:, iq), vr, g2kin, psi, eval_occ(:,iq) ) 
 ...
 !@   write ( stdout, '(4x,"k(",i3," )",10(3x,f7.3))') iq, eval_occ(:,iq)*ryd2ev
   write ( stdout, '(4x,"k(",i3," )",10(3x,f7.3))') iq, eval_all*ryd2ev
    !
  enddo
!@
stop
